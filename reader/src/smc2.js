...
// Tabel variabel LOKAL tiap program = entity `Variables` anak entity `Program`. Tanpa ini
// semua variabel lokal tercampur jadi satu daftar, dan `LB411` ST3 terbaca dengan komen
// `LB411` milik ST1 - nama sama, arti lain, tanpa satu pun tanda.
function varOwners(oemText) {
  const own = new Map();
  function walk(node, prog) {
    const t = node.attrs.type;
    if (t === 'Program') prog = node.attrs.name || prog;
    else if (t === 'Variables' && prog && node.attrs.id) own.set(node.attrs.id, prog);
    const kids = xmlChild(node, 'ChildEntities');
    if (kids) for (const ch of kids.kids) if (ch.tag === 'Entity') walk(ch, prog);
  }
  for (const e of xmlParse(oemText).kids) if (e.tag === 'Entity') walk(e, null);
  return own;
}
...
async function readProject(buf, unzipFn) {
  ...
  const oemText = await get(oemName);
  const owners = varOwners(oemText);
  const seen = new Map();
  for (const t of parseTree(oemText)) {
  ...
  for (const [n, f] of files) {
    ...
    if (b.length > 6 && text(b.subarray(0, 8)).startsWith('[SLWD ')) {
      const prog = owners.get(n.split('/').pop().replace(/\.xml$/, ''));
      const vs = parseVars(text(b));
      if (prog) vs.forEach(v => { if ((v.group || '').indexOf('GLOBAL') < 0) v.program = prog; });
      proj.variables.push(...vs);
    }
  }
  return proj;
}
