...
// Baris array-level (AL, MF) ada di GlobalVariables.tsv; komen PER ELEMEN (AL[11]) pindah ke
// ArrayComments.tsv - berkas global sengaja tidak memuatnya lagi, karena elemen array baru bisa
// ditempel setelah arraynya di-expand di Sysmac.
let p=gen(null);
let tsv=p.files.find(f=>f.name==='GlobalVariables.tsv').xml;
let el=p.files.find(f=>f.name==='ArrayComments.tsv').xml;
...
chk('tiap elemen array punya komen (gak ada yang kosong)',
    count(el,'AL')===p.arrayInfo.alSize && count(el,'MF')===p.arrayInfo.mfSize,
    'AL '+count(el,'AL')+'/'+p.arrayInfo.alSize+'  MF '+count(el,'MF')+'/'+p.arrayInfo.mfSize);
...
el=p.files.find(f=>f.name==='ArrayComments.tsv').xml;
chk('ukuran custom kepakai', arrType(tsv,'AL')==='ARRAY[1..250] OF BOOL' && arrType(tsv,'MF')==='ARRAY[1..120] OF BOOL',
    arrType(tsv,'AL')+' | '+arrType(tsv,'MF'));
chk('elemen komen ikut nambah', count(el,'AL')===250 && count(el,'MF')===120, count(el,'AL')+' / '+count(el,'MF'));
chk('stub bernomor sampai ujung', /AL250_ Spare/.test(el) && /MF120_ Spare/.test(el));
...
chk('slot terakhir yang kepakai tetap ada isinya',
    el.includes('AL['+p.arrayInfo.alUsed+']') && !new RegExp('AL\\['+(p.arrayInfo.alUsed+1)+'\\]').test(el));
...
chk('array ngikut, gak nyisain slot tanpa komen', count(el,'AL')===p.arrayInfo.alSize,
    count(el,'AL')+'/'+p.arrayInfo.alSize);
...
const base=gen(null).files.find(f=>f.name==='ArrayComments.tsv').xml;
...
let t=genIO(IO_MORE_ACT).files.find(f=>f.name==='ArrayComments.tsv').xml;
...
const p4=genIO(IO_MORE_UNIT); t=p4.files.find(f=>f.name==='ArrayComments.tsv').xml;
...
t=genIO(IO_GAP).files.find(f=>f.name==='ArrayComments.tsv').xml;
