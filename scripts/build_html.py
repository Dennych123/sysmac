...
function buildNbPanel(files) {
  ...
  var hint = document.createElement('div'); hint.className = 'hint';
  hint.textContent = 'Unduh, lalu di NB-Designer buka Alarm Setting dan tekan Import. BUKAN dengan '
    + 'menyalin berkas ke folder project - alarm NB tersimpan di dalam .nbp, bukan dibaca dari folder. '
    + 'Import MENGGANTI seluruh daftar alarm, jadi tekan Export dulu kalau yang sekarang masih dipakai.';
...
function nbCmdText() {
  // Placeholder ditulis dalam kurung siku supaya tidak ada yang menyalinnya bulat-bulat lalu
  // dapat "project.json tidak ditemukan" - itu kejadian. Contoh path Windows sengaja dibuang:
  // backslash-nya harus di-escape dobel di template ini, dan itu jebakan yang tidak sepadan.
  return 'node scripts/nb_apply.js <AlarmLib.csv yang diunduh> <folder project NB> --write';
}
function nbCopyCmd() {
  var text = nbCmdText();
  function done(ok) {
